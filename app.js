...
let Menusett=new Set(["All"])

let MenDOM=document.querySelector('.btn-container')
let btnStyle = `
...
menu.forEach(function(element){
  Menusett.add(element.category)
})

Menusett.forEach(function(element){
  let butonDOM=document.createElement('span')
  butonDOM.classList.add("btn-item")
  butonDOM.style=btnStyle
  butonDOM.setAttribute('id',`${element}`)
  butonDOM.innerHTML=`${element}`
  MenDOM.appendChild(butonDOM)
})

let fastFoodDOM=document.querySelector('.section-center')
menu.forEach(AddList)
function AddList(element){
  let div=document.createElement('div')
  div.classList.add("menu-items","col-lg-6","col-sm-12")
  div.setAttribute('name',element.category)
